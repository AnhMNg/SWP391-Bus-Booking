...
public class TicketManager {
    ...
    public static void changeTicket(long ticketIdRemove, long orderId, long routeDetailId, int position, String passengerName, String passengerPhone) throws SQLException {
        Connection cn = DBUtils.getConnection();
        if (cn != null) {
            String sql = "INSERT INTO Ticket VALUES(?,?,?, ?, ?, CURRENT_TIMESTAMP)\n"
            ...
            pst.setNString(4, passengerName);
            pst.setString(5, passengerPhone);
            pst.setLong(6, ticketIdRemove);
            pst.executeUpdate();
            pst.close();
            cn.close();
        }
    }
    public static TicketDetail getTicketById(long id){
    Connection cn = null;
        TicketDetail tik = null;
        try {
            cn = DBUtils.getConnection();
            if (cn != null) {
                String sql = "select rd.price,rd.startTime,rd.timeArrival, com.name, bt.capacity, bt.kind,PlaceName.depart,PlaceName.destination,rd.departDetail, rd.detinationDetail,tk.ticketId,tk.orderId,tk.routeDeTailId, tk.position, tk.passengerName,tk.passengerPhone, tk.purchaseDate\n"
                        ...
                        + "                        (select * from Route,Place where Route.destinationId = Place.placeId) des\n"
                        + "                        where des.routeId = dep.routeId) PlaceName\n"
                        + "                        where tk.orderId = od.orderId and tk.routeDeTailId = rd.routeDetailId and rd.routeId = r.routeId and tk.ticketId = ?\n"
                        + "                        		and rd.busTypeId = bt.busTypeId and com.companyId = r.companyId and PlaceName.routeId = r.routeId";
                PreparedStatement pst = cn.prepareStatement(sql);
                pst.setLong(1, id);
                ResultSet rs = pst.executeQuery();
                if (rs != null && rs.next()) {
